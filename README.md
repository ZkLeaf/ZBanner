...
|setBannerTitleList(List<String> titles)| 设置轮播要显示的标题和图片对应（如果不传默认不显示标题）|无
...
|setImages(Object[]/List<?> imagesUrl,OnLoadImageListener listener)| 设置轮播图片，并且自定义图片加载方式|无
|setOnBannerListener(this)|设置点击事件，下标是从0开始|无
|setOnLoadImageListener(this)|设置图片加载事件，可以自定义图片加载方式|无
...
```groovy
dependencies{
    implementation 'com.github.ZkLeaf:ZBanner:v1.1.0'  //最新版本
}
```
或者引用本地lib
```groovy
...
public class GlideImageLoader extends ImageLoader {
    @Override
    public void displayImage(Context context, Object path, ImageView imageView, OnVideoStateListener listener) {
        /**
          注意：
